<div class="page-inner">
    <div class="page-header">
        <h4 class="page-title">Detail Ket. Penghasilan</h4>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        ...
                        <a href="<?= base_url('admin/surat_penghasilan'); ?>" class="btn btn-secondary btn-round ml-auto">
                            <i class="fas fa-arrow-left"></i> Kembali
                        </a>
                    </div>
                </div>
                <div class="card-body">
                    <?php $bisaCetak = $bisaCetak ?? (!empty($surat->nomor_surat) && $surat->status == 'Disetujui'); ?>
                    <?php if (!$bisaCetak): ?>
                        <div class="alert alert-warning">
                            <h4 class="alert-heading"><i class="fas fa-exclamation-triangle"></i> Surat Belum Siap Cetak!</h4>
                            <p class="mb-0">Pastikan <b>Nomor Surat</b> sudah diisi dan <b>Status</b> sudah "Disetujui".</p>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <dl class="row">
                                ...
                                <dd class="col-sm-8">:
                                    <?php $badge = $surat->status == 'Pending' ? 'badge-warning' : ($surat->status == 'Disetujui' ? 'badge-success' : 'badge-danger'); ?>
                                    ...
                                    <?php if (!empty($files)): ?>
                                        <ul class="list-unstyled">
                                            <?php foreach ($files as $fn): ?>
                                                <li class="mb-1"><i class="fa fa-paperclip"></i> <a href="<?= base_url('uploads/pendukung/' . $fn) ?>" target="_blank" rel="noopener"><?= html_escape($fn) ?></a></li>
                                            <?php endforeach; ?>
                                        </ul>
                                    <?php else: ?><span class="text-muted">Tidak ada dokumen terunggah.</span><?php endif; ?>
                                </dd>
                            </dl>
                        </div>
                    </div>
                ...
                <div class="card-footer">
                    <a href="<?= base_url('admin/surat_penghasilan/edit/' . $surat->id); ?>" class="btn btn-warning"><i class="fa fa-edit"></i> Edit Data Ini</a>
                    <?php if ($bisaCetak): ?>
                        <?php if (!empty($signers)): ?>
                            <form class="form-inline d-inline-flex align-items-center ml-2"
                                action="<?= base_url('admin/surat_penghasilan/cetak/' . $surat->id); ?>" method="get" target="_blank">
                                <button type="submit" class="btn btn-success mr-2"><i class="fa fa-print"></i> Cetak Surat (PDF)</button>
                                ...
                                <select name="ttd" id="ttd" class="form-control" style="min-width:320px" required>
                                    <?php foreach ($signers as $s): ?>
                                        <option value="<?= (int)$s->id; ?>" <?= ($default_signer_id && (int)$default_signer_id === (int)$s->id) ? 'selected' : ''; ?>>
                        ...
                        <?php endif; ?>
                    <?php else: ?>
                        <button class="btn btn-success" disabled title="Lengkapi data di halaman edit terlebih dahulu"><i class="fa fa-print"></i> Cetak Surat (PDF)</button>
                    <?php endif; ?>
                </div>
            </div>
        </div>
