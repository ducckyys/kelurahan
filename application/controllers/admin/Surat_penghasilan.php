 ...
 * @property CI_PDF $pdf
 */
class Surat_penghasilan extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(['M_penghasilan', 'M_pejabat']);
    ...
    {
        $data['title'] = "Data Surat Keterangan Penghasilan";
        $data['list']  = $this->M_penghasilan->get_all();
        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar', $data);
        $this->load->view('admin/surat_penghasilan/v_list', $data);
        $this->load->view('layouts/footer');
    }

    public function detail($id)
    {
        $data['surat'] = $this->M_penghasilan->get_by_id($id);
        if (!$data['surat']) return redirect('admin/surat_penghasilan');
        ...
        $data['title'] = "Detail Surat Keterangan Penghasilan";
        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar', $data);
        $this->load->view('admin/surat_penghasilan/v_detail', $data);
        $this->load->view('layouts/footer');
    }
    ...
    public function edit($id)
    {
        ...
        $data['title']         = "Edit Surat Keterangan Penghasilan";
        $data['can_full_edit'] = $this->is_superadmin();

        $this->load->view('layouts/header', $data);
        $this->load->view('layouts/sidebar', $data);
        $this->load->view('admin/surat_penghasilan/v_edit', $data);
        $this->load->view('layouts/footer');
    }

    private function upload_multiple_from_admin()
    {
        if (empty($_FILES['dokumen_pendukung']['name']) || empty($_FILES['dokumen_pendukung']['name'][0])) return [];
        ...
        for ($i = 0; $i < $count; $i++) {
            if ($files['error'][$i] !== UPLOAD_ERR_OK) {
                $this->session->set_flashdata('error', 'Gagal unggah dokumen (error ' . $files['error'][$i] . ').');
                return false;
            }
            $_FILES['single'] = ['name' => $files['name'][$i], 'type' => $files['type'][$i], 'tmp_name' => $files['tmp_name'][$i], 'error' => $files['error'][$i], 'size' => $files['size'][$i]];
            $config = ['upload_path' => $this->pendukung_dir, 'allowed_types' => $allowed, 'max_size' => $max_kb, 'encrypt_name' => TRUE];
    ...
    public function update($id)
    {
        if (!$this->is_superadmin()) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                return redirect('admin/surat_penghasilan/edit/' . $id);
            }
            $data = ['nomor_surat' => $this->input->post('nomor_surat', true) ?: null, 'status' => $this->input->post('status', true)];
            $this->db->where('id', $id)->update('surat_penghasilan', $data);
            $this->session->set_flashdata('success', 'Status / Nomor surat berhasil diperbarui.');
            return redirect('admin/surat_penghasilan/detail/' . $id);
        }
        ...
        $row = $this->M_penghasilan->get_by_id($id);
        $existing = [];
        ...
        if ($newFiles === false) return redirect('admin/surat_penghasilan/edit/' . $id);
    ...
    public function delete($id)
    {
        if ($this->session->userdata('id_level') !== '1') {
            ...
            return redirect('admin/surat_penghasilan');
        }
        $surat = $this->M_penghasilan->get_by_id($id);
        ...
        $this->M_penghasilan->delete($id);
        $this->session->set_flashdata('success', 'Data berhasil dihapus.');
        redirect('admin/surat_penghasilan');
    }
    ...
    public function cetak($id)
    {
        ...
        if ($data['surat']->status != 'Disetujui') {
            $this->session->set_flashdata('error', 'Gagal cetak! Status harus Disetujui.');
            return redirect('admin/surat_penghasilan/edit/' . $id);
        }
        ...
        $data['title'] = "Cetak Surat Keterangan Penghasilan - " . $data['surat']->nama_pemohon;
        $filename = 'surat-penghasilan-' . $data['surat']->nik;

        $html = $this->load->view('admin/surat_penghasilan/v_cetak', $data, true);
