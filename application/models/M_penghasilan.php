...
class M_penghasilan extends CI_Model
{
    ...
    public function set_pendukung($id, array $files)
    {
        ...
        return $this->update($id, ['dokumen_pendukung' => !empty($clean) ? json_encode($clean) : null]);
    }
    ...
    public function remove_pendukung_file($id, $filename)
    {
        ...
        $filtered = array_values(array_filter($files, fn($f) => $f !== $filename));
